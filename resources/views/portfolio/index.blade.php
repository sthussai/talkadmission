...
<div class="max-w-[800px] mx-auto flex items-center justify-center min-h-screen m-4 mt-40 text-[#ecebf0]">
        <div
            ...
            <a href="{{route('portfolio.edit', auth()->user()->id )}}"
                class="absolute text-[#231f39] bg-gray-200 rounded-[4px] hover:bg-green-200 top-6 right-6 px-4 py-2 text-sm font-bold roll-in-blurred-right">
                Edit
            </a>
            ...
            <div class="flex flex-col items-center -mt-20">
                <a href="/portfolio" target="_blank" class="">
                    <img src="{{ Auth::user()->avatar_url }}"
                        class="w-40 border-6 border-white rounded-full ">
                </a>
                <h1 class="text-xl font-bold text-center">{{auth()->user()->name ?? 'John Doe'}}</h1>
                <p class="block my-1 text-center">{{'Class of '. $portfolio->grad_year ?? '' }}</p>
                <p class="mt-5 text-center">{{$portfolio->med_school ?? '' }}</p>
            </div>
            ...
            <div class="mt-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
                <div class="w-full flex flex-col 2xl:w-1/3">
                    <div class="flex-1 bg-white  n shadow-xl p-8">
                        ...
                        <ul class="mt-2 text-gray-700">
                            <li class="flex border-y py-2">
                                ...
                                <span class="text-gray-700">{{$portfolio->previous_degree ?? '' }}</span>
                            </li>
                            <li class="flex border-b py-2">
                                <span class="font-bold mr-2">MMIs attended as pre-med:</span>
                                <span class="text-gray-700">{{$portfolio->mmis_attended ?? '' }}</span>
                            </li>
                            <li class="flex border-b py-2">
                                <span class="font-bold mr-2">MMIs as an interviewer:</span>
                                <span class="text-gray-700">{{$portfolio->mmis_interviewed?? '' }}</span>
                            </li>
                            <li class="flex border-b py-2">
                                <span class="font-bold mr-2">Mentorship experience:</span>
                                <span class="text-gray-700">{{$portfolio->mentorship_experience ?? '' }}</span>
                            </li>
                            <li class="flex border-b py-2">
                                <span class="font-bold mr-2">Applicants helped:</span>
                                <span class="text-gray-700">{{$portfolio->applicants_helped ?? '' }}</span>
                            </li>
                    ...
                    <div class="bg-gray-100 text-gray-600 p-4 text-sm font-semibold backdrop-blur-sm">
                        ...
                        {{$portfolio->about ?? 'No details entered' }}
                        </p>
                    </div>
